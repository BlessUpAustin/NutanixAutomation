...
class TestRestApi():                
  ...
  #Delete the protection domain schedule
  def PDScheduleCleanup(self):
    PDScheduleURL = self.base_url + "/protection_domains/testPD/schedules"
    print("Deleting protection domain schedule on cluster %s" % self.serverIpAddress)
    serverResponse = self.session.delete(PDScheduleURL)
    print("Response code: %s" % serverResponse.status_code)
...
if __name__ == "__main__":
    ...
    try:
      ...
      #Delete protection domain schedule
      deletePDSchedule = testRestApi.PDScheduleCleanup()
      print ("Text: ")
      pp.pprint(deletePDSchedule)
      print(("=" * 79))
      print(("=" * 79))
      print(("=" * 79)) 
